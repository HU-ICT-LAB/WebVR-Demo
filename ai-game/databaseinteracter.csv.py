...
def txt_getlastmovement(databoard=True):
    """
...
def csv_getsorted_data():
    ...
    def get_total_controllers_distance(data):
        ...
        for index, row in data.iterrows():
            ...
            totaldist += dist
            leftpunchspeed = np.linalg.norm(p1 - p2)*2 #(to turn meters/half a second to meter/second
            # right
            ...
            squared_dist = np.sum((p1 - p2) ** 2, axis=0)
            dist = np.sqrt(squared_dist)
            rightpunchspeed = np.linalg.norm(p1 - p2)*2
            if dist > 0.6:
            ...
            if leftpunchspeed > highestspeed:
                highestspeed = rightpunchspeed
            elif (leftpunchspeed < slowestspeed or slowestspeed == 0) and leftpunchspeed > 0.09:
                slowestspeed = rightpunchspeed
            formertime = row['time']
            totaldist += dist
        return totaldist, total_time, round(highestspeed, 2), round(slowestspeed, 2)
    ...
    return lastgame_total_controller_distance, lastgame_movingtime, lastgame_calories_burned, allgames_total_controller_distance, highestspeed, slowestspeed
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == "hbo_ict_vr_data_last_movement":
        ...
        if txt_getlastmovement(False) != str(
                last_position) and gamestarted == "true":  # prevents saving stationary movements
            csv_add_move_data(name, txt_getlastmovement(False), last_position)  # compare current and former position
    ...
    if msg.topic == "hbo_ict_vr_request_database":
        ...
        client3.publish('hbo_ict_vr_request_simplified_lastmovement', txt_getlastmovement(databoard=True))

    if msg.topic == "hbo_ict_vr_request_database_aftergame":
        '''pushes all aftergame data'''
        lastgame_total_controller_distance, lastgame_movingtime, lastgame_calories_burned, allgames_total_controller_distance, allgames_total_fastest, allgames_total_slowest = csv_getsorted_data()
        client3.publish('hbo_ict_vr_request_simplified_lastgame_total_controller_distance',
                        lastgame_total_controller_distance)
        ...
        client3.publish('hbo_ict_vr_request_simplified_allgames_total_controller_distance',
                        allgames_total_controller_distance)
        client3.publish('hbo_ict_vr_request_simplified_allgames_slowest_punch',
